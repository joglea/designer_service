  ...
  DateTimePicker.prototype = {
    ...
    init: function(element, options) {
      ...
      if (this.pickTime) {
        ...
        if (!this.timeIcon) this.timeIcon = 'fa-clock-o';
        icon.addClass(this.timeIcon);
      }
      if (this.pickDate) {
        if (icon && icon.length) this.dateIcon = icon.data('date-icon');
        if (!this.dateIcon) this.dateIcon = 'fa-calendar';
    ...
    set: function() {
      ...
      if (!this.isInput) {
        ...
        //定制  input改变时  出发change事件
        if(this.$element.change!=undefined)
        {
            this.$element.change();
        }
      } else {
        this.$element.val(formatted);alert(3);
        ...
        '<div class="bootstrap-datetimepicker-widget dropdown-menu">' +
          '<ul>' +
            '<li' + (collapse ? ' class="collapse in"' : '') + '>' +
              ...
              '</div>' +
            '</li>' +
            '<li class="picker-switch accordion-toggle"><a style="width:100%;" class="btn btn-primary"><i class="fa ' + timeIcon + '"></i></a></li>' +
